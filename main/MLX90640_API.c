...
{
     return IR_I2CRead(slaveAddr, 0x2400, 832, eeData);
}
...
int MLX90640_GetSubFrameData(uint8_t slaveAddr, uint16_t *frameData)
{
    ...
    while(dataReady == 0)
    {
        error = IR_I2CRead(slaveAddr, 0x8000, 1, &statusRegister);
        if(error != 0)
        {
            return error;
        }    
    ...
    error = IR_I2CWrite(slaveAddr, 0x8000, 0x0030);
    if(error == -1)
    {
        return error;
    }
    ...
    //reading frame
    error = IR_I2CRead(slaveAddr, 0x0400, 832, frameData); 
    if(error != 0)
    {
        return error;
    }
    
    error = IR_I2CRead(slaveAddr, 0x800D, 1, &controlRegister1);
...
int MLX90640_GetFrameData(uint8_t slaveAddr, uint16_t *frameData)
{
    ...
    if(cnt > 4)
    {
       ESP_LOGI("IR","timeout in reading second page");
        return -8;
    }    
...
int MLX90640_SetResolution(uint8_t slaveAddr, uint8_t resolution)
{
    ...
    if(error == 0)
    {
        ...
        error = IR_I2CWrite(slaveAddr, 0x800D, value);        
    }    
