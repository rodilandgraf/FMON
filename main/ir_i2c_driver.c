...
{   
    ...
    {
        ESP_LOGE("IR", "I2C INSTALL ERROR %i %i", cret, iret);
    }
}
...
int IR_I2CRead(uint8_t slaveAddr, uint16_t startAddress, uint16_t nMemAddressRead, uint16_t *data)
{
    ...
    if(ret != ESP_OK)
    {
        ESP_LOGE("IR", "I2C READ ERROR %i", ret);
    }
    return ret;   
} 
...
int IR_I2CWrite(uint8_t slaveAddr, uint16_t writeAddress, uint16_t data)
{
    ...
    if(ret != ESP_OK)
    {
        ESP_LOGE("IR", "I2C WRITE ERROR");
    }
    
    IR_I2CRead(slaveAddr,writeAddress,1, &dataCheck);
    if ( dataCheck != data)
    {
        return -2;
    }   
